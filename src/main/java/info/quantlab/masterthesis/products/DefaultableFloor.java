...
/**
 * This class implements the valuation of a defaultable floor.
 * The payoff of the product is ( <i>K<sub>i</sub> - L(T<sub>i</sub>) </i> )<sup>+</sup>, for <i>i = 0, 1, ... n-1 </i>
 * and <i> n = </i><code>fixingTimes.length</code>.<p/>
 * If the valuation TermStructureModel is non defaultable, this class behaves like a normal floor.
 *
 * @author Markus Parhofer
 * @version 1.0
 */
public class DefaultableFloor extends AbstractSellerDefaultableTermStructureProduct {
	 ...
	 * 
	 * @param fixingTimes The vector of fixing times
	 * @param paymentTimes The vector of payment times
	 ...
	 * @param notional The notional.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double[] strikes, double notional) {
	 ...
	 * @param notional The notional.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double strike, double notional) {
	...
	public RandomVariable getValue(double evaluationTime, TermStructureMonteCarloSimulationModel model)	throws CalculationException {
		...
		for(int period=0; period<_fixingTimes.length; period++)
		{
			...
			// Calculate payout
			RandomVariable payoff = libor.bus(strike).mult(periodLength).floor(0.0);
