...
/**
 * This class implements the valuation of a floor where a possibility for default is accounted for.
 * The payoff of the product is ( <i> K<sub>i</sub></i> -  L(T<sub>i</sub>))<sup>+</sup>, for <i>i = 0, 1, ... n-1 </i>
 ...
 * @version 1.0
 */
public class DefaultableFloor extends AbstractDefaultableTermStructureProduct {
	...
	/**
	 * Constructs a Floor on a LIBOR rate, where one can specify if a defaultable Model should be used for the underlying or for the issuer and 
	 * if so, which one. 
	 * See Java Doc on {@link DefaultableFloor} for more information.
	 * 
	 ...
	 */
	public DefaultableFloor(double[] fixingTimes, double[] paymentTimes, double[] strikes, int modelIndexOfUnderlying, int modelIndexOfIssuer, double notional) {
	...
	/**
	 * Constructs a Floor on a LIBOR rate, where one can specify if a defaultable Model should be used for the underlying or for the issuer.
	 ...
	 * @param notional The notional
	 */
	public DefaultableFloor(double[] fixingTimes, double[] paymentTimes, double[] strikes, boolean useDefaultableUnderlying, boolean useDefaultableIssuer, double notional) {
		this(fixingTimes, paymentTimes, strikes, useDefaultableUnderlying ? 0 : -1, useDefaultableIssuer ? 0 : -1, notional);
	}
	
	/**
	 * Constructs a Floor on a defaultable LIBOR rate, but without issuer risk (i.e. modelIndexOfUnderlying is 0 and modelIndexOfIssuer is -1).
	 * 
	 * @param fixingTimes The array of fixing times
	 * @param paymentTimes The array of payment times
	 * @param strikes The array of strikes
	 * @param notional The notional
	 */
	public DefaultableFloor(double[] fixingTimes, double[] paymentTimes, double[] strikes, double notional) {
	 ...
	 * @param modelIndexOfIssuer The index for the defaultable model to use as the issuer. Set -1 for non-defaultable model.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double[] strikes, int modelIndexOfUnderlying, int modelIndexOfIssuer) {
	 ...
	 * @param useDefaultableIssuer Specifies, if the issuer is defaultable. If so, the default model index (0) is used.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double[] strikes, boolean useDefaultableUnderlying, boolean useDefaultableIssuer) {
	 ...
	 * @param strikes The array of strikes.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double[] strikes) {
	 ...
	 * @param modelIndexOfIssuer The index for the defaultable model to use as the issuer. Set -1 for non-defaultable model.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double strike, int modelIndexOfUnderlying, int modelIndexOfIssuer) {
	 ...
	 * @param useDefaultableIssuer Specifies, if the issuer is defaultable. If so, the default model index (0) is used.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double strike, boolean useDefaultableUnderlying, boolean useDefaultableIssuer) {
	 ...
	 * @param strike The strike. Sets all strikes to the same value.
	 */
	public DefaultableFloor(TimeDiscretization tenor, double strike) {
	...
	public RandomVariable getValue(double evaluationTime, TermStructureMonteCarloSimulationModel model)	throws CalculationException {
		...
		for(int period = 0; period < _fixingTimes.length; period++)
		{
			...
			// Calculate payout
			RandomVariable payoff = forwardRate.bus(strike).mult(periodLength).floor(0.0).mult(_notional);
